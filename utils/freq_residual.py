...
def compute_phase_congruency(image: np.ndarray, sigma=2.0) -> np.ndarray:
    """
    计算图像的相位一致性（Phase Congruency）近似值。
    使用 Gabor 滤波器组模拟多尺度边缘响应。
    返回: (H, W) 相位一致性图（值 ∈ [0,1]）
    """
...
def extract_frequency_residual(image: np.ndarray, target_size=224) -> np.ndarray:
    ...
    提取频域残差特征：
      1. 计算相位一致性图（PC）
      2. 对 PC 图做 DCT，取高频块能量（作为“残差”）
      3. 输出：(H_b, W_b, 1) → 单通道高频残差图
    """
    if image.dtype == np.uint8:
    ...
    # Step 2: 对 PC 图做 DCT 分块（8x8），取高频能量
    h, w = pc_map.shape
    block_size = 8
    ...
    for i in range(0, h_crop, block_size):
        ...
        for j in range(0, w_crop, block_size):
            ...
            high_freq = dct_block[4:, 4:]  # 4x4 高频
